...
export const CLAW = {
  ...
  /** 흔들림 때문에 아주 가끔 놓친다 (초당 확률). 아슬아슬할수록 최대 3배까지 오른다. */
  slipPerSec: 0.04,
  /**
   * ④ 미끄러짐이 눈에 보이기 시작하는 부하 비율 (토크 / 버티는 힘).
   *    이 위로는 인형이 집게 안에서 조금씩 돌아가고 집게가 떨린다.
   *    낮출수록 위태로운 장면이 자주 보인다.
   */
  slipVisibleFrom: 0.55,
  /** 미끄러질 때 인형에 먹이는 회전력. 키우면 더 크게 휘청인다. */
  slipTorque: 0.9,
